...
`Tabs` supports per-tab colors, a single background color, a uniform outline
around the active tab and content, separate border radii for the content and
tabs, tab placement on any edge, tab range constraints, text style animation,
custom transition builders, tab-strip action buttons, and child padding.
