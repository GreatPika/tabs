...
labels on any tab-strip edge. `unselectedTabGap` controls the space between
labels and defaults to 2 logical pixels. The gaps remain transparent, and each
unselected label uses the same shape and corner radii as the active label.
Leaving `unselectedTabColor` unset preserves the previous appearance.
