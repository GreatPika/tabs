...
Tabs(
  ...
  unselectedTabColor: Colors.blueGrey,
...
Use `border` with a uniform [Border.all] to draw one outline around the active
tab and its content. The border follows the same joined shape as the active
background.

Set `unselectedTabColor` to opt into separate tab surfaces for unselected
labels on the top or bottom tab strip. `unselectedTabGap` controls the space
between labels and defaults to 2 logical pixels. The gaps remain transparent,
and each unselected label uses the same shape and corner radii as the active
label.
`unselectedTabShadow` can add separation without changing that shape. Leaving
